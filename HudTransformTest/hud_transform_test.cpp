...
static constexpr float HUD_TRANSLATE_X = 5.25f;
static constexpr float HUD_TRANSLATE_Y = 0.0f;
static constexpr float HUD_TRANSLATE_Z = 0.0f;
...
{
    char path[MAX_PATH] = "/home/cyril/Downloads/hud_transform_test.log\0";
...
static void WINAPI HookApplyMatrix(uint32_t handle, float* matrix)
{
    ...
    if (returnAddress == g_hudCallReturnAddress && matrix)
    {
        matrix[12] += HUD_TRANSLATE_X;
        matrix[13] += HUD_TRANSLATE_Y;
        matrix[14] += HUD_TRANSLATE_Z;

        LONG count = InterlockedIncrement(&g_hudHitCount);
        if (count <= 20 || (count % 300) == 0)
        {
            LogFormat(
                "HUD matrix hit #%ld -- handle=0x%08x pos=(%.6f, %.6f, %.6f)\r\n",
                count,
                handle,
                matrix[12],
                matrix[13],
